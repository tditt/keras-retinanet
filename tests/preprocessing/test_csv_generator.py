...
def test_read_annotations():
    ...
    annotations = csv_generator.read_annotations(csv_str(
        'a.png,0,1,2,3,a'     '\n'
        'b.png,4,5,6,7,b'     '\n'
        'c.png,8,9,10,11,c'   '\n'
        'd.png,12,13,14,15,d' '\n'
...
def test_read_annotations_wrong_format():
    ...
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str(
                'a.png,0,1,2,3,a' '\n'
                'a.png,1,2,3,a'   '\n'
            ), classes)
        except ValueError as e:
...
def test_read_annotations_wrong_x1():
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,a,0,1,2,a'), {'a': 1})
        except ValueError as e:
...
def test_read_annotations_wrong_y1():
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,0,a,1,2,a'), {'a': 1})
        except ValueError as e:
...
def test_read_annotations_wrong_x2():
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,0,1,a,2,a'), {'a': 1})
        except ValueError as e:
...
def test_read_annotations_wrong_y2():
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,0,1,2,a,a'), {'a': 1})
        except ValueError as e:
...
def test_read_annotations_wrong_class():
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,0,1,2,3,g'), {'a': 1})
        except ValueError as e:
...
def test_read_annotations_invalid_bb_x():
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,1,2,1,3,g'), {'a': 1})
        except ValueError as e:
            assert str(e).startswith("line 1: x2 (1) must be higher than x1 (1)")
            raise
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,9,2,5,3,g'), {'a': 1})
        except ValueError as e:
...
def test_read_annotations_invalid_bb_y():
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,1,2,3,2,a'), {'a': 1})
        except ValueError as e:
            assert str(e).startswith("line 1: y2 (2) must be higher than y1 (2)")
            raise
    with pytest.raises(ValueError):
        try:
            csv_generator.read_annotations(csv_str('a.png,1,8,3,5,a'), {'a': 1})
        except ValueError as e:
...
def test_read_annotations_empty_image():
    ...
    assert csv_generator.read_annotations(csv_str('a.png,0,1,2,3,a\na.png,,,,,'), {'a': 1}) == {'a.png': [annotation(0, 1, 2, 3, 'a')]}
